...
|data>>is_points_behind|是否分抛计算：<br>0:否,<br>1:是|是|[int]|1|
...
|data>>is_print|是否打印拣货单：<br>0：未打印，<br>1：已打印|是|[int]|0|
|data>>print_num|打印次数|是|[int]|0|
|data>>is_pick|是否拣货：<br>0：未拣货，<br>1:已拣货|是|[int]|0|
...
|data>>is_exist_declaration|是否关联报关单，<br>0：否，<br>1：是|是|[int]| |
|data>>is_exist_clearance|是否关联清关单，<br>0：否，<br>1：是|是|[int]| |
|data>>is_relate_aux|是否关联辅料，<br>0：否，<br>1：是|是|[int]|1|
|data>>third_party_order_mode|下单模式，<br>0：无，<br>1：系统下单，<br>2：手工下单|否|[int]| |
|data>>third_party_logistics_wp_code|第三方仓服务商代码，如Gucang、Xyzc|否|[string]| |
|data>>third_party_order_status|第三方仓下单状态，待发货下才有，<br>1：未下单，<br>2：已下单，<br>3：异常，<br>4：已发货|否|[int]| |
...
|data>>is_change_label|（谷仓）换标服务，<br>0：不换标，<br>1：换标|否|[int]| |
|data>>label_replacement_option|（谷仓）换标要求，<br>1：外箱，<br>2：内箱|否|[int]| |
|data>>is_signature|（谷仓/西邮智仓）签名服务，<br>0：不选择签名服务，<br>1：签名服务|否|[int]| |
...
|data>>ware_operation_type|（西邮智仓）库内操作，<br>0：无，<br>1换条码，<br>2:换箱唛，<br>3换条码换箱唛|否|[int]| |
|data>>other_ware_operation|（西邮智仓）其他库内操作，<br>0：无，<br>1换托唛|否|[int]| |
|data>>inventory_type|（西邮智仓）库存类型，<br>0：标准，<br>1：不良品，<br>4：退货|否|[int]| |
|data>>order_cod_currency|（西邮智仓）下单币种|否|[string]| |
|data>>is_split_prediction|（西邮智仓）是否拆单预报，<br>0：否，<br>1：是|否|[int]| |
|data>>is_custom_cost|是否自定义成本，<br>1：是，<br>0：否|是|[int]| |
|data>>pick_num|已拣货数量|否|[int]| |
|data>>vat_code|店铺VAT税号|否|[string]| |
|data>>head_fee_status|头程分摊相关规格填写状态，<br>0：未完整填写，<br>1：已完整填写|否|[int]| |
|data>>is_custom_shipment_time|是否自定义发货时间，1：是，0：否|否|[int]| |
|data>>return_stock_type|恢复库存类型：<br>0未恢复；<br>1全部恢复；<br>2产品库存；<br>3辅料库存|否|[int]| |
|data>>cancel_reason|作废理由|否|[string]| |
|data>>packing_task_sn|装箱任务编号|否|[string]| |
|data>>is_auto_adjust|是否自动换标调整，<br>1：是，0：否|否|[int]| |
...
|data>>is_relate_head_logistics|是否关联头程物流, <br>0:否,<br>1:是|否|[int]| |
...
|data>>items>>cbm|CBM，商品体积，非装箱信息体积|是|[string]|0.00|
...
|data>>items>>packing_type|混装类型：<br>1：原装，<br>2：原装|是|[int]|1|
...
|data>>items>>is_sta|是否sta货件，<br>1：是，<br>0：否|是|[string]| |
...
|data>>auxs>>aux_head_fee_type|辅料分摊类型 <br>1:按sku分摊,<br>2:实重分摊,<br>3:按体积重分摊,<br>4:按计费重分摊|是|[int]|3|
...
|data>>logistics_list_type|物流信息版本<br>0旧版<br>1新版|是|[object]|0|
...
|data>>head_logistics_list>>track_list>>transport_type|运输类型 ：<br>1快递；<br>2海运；<br>3空运；<br>4其他；|是|[string]|1|
...
|data>>head_logistics_list>>track_list>>order_type_code|单号类型：<br>1.订舱号；<br>2.提单号；<br>3.箱号；<br>4.其他；<br>5.跟踪单号；<br>6航班号<br> （注意：与运输类型有联动） 当transport_type=1只能传<br> 5跟踪单号 <br> 当transport_type=2只能传 <br>1.订舱号 <br>2.提单号 <br>3.箱号<br> 4.其他<br> transport_type=3只能传 <br>2.提单号 <br>6.航班号<br> 当transport_type=4只能传 空|是|[string]|跟踪单号|
...
|data>>box_list>>box_skus|箱子内包含的SKU信息，<br>SINGLE类型装箱，只会有一个子项，<br>MULTIPLE类型装箱，可能会有多个子项|是|[array]| |
