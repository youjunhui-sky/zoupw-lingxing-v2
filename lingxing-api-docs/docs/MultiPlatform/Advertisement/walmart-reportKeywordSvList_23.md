...
| :------------ |:-----| :------------ | :------------ |
| `/basicOpen/multiplatform/ads/reportKeywordSvList` | HTTPS | POST | 1 |
...
| adGroupIds | 广告组ID列表，Integer数组，按广告组ID筛选 | 否 | [array] | [123456789,987654321] |
...
| searchText | 搜索文本，模糊搜索关键词文本（keyword_text） | 否 | [string] | 手机壳 |
| status | 关键词状态列表，String数组，枚举值：enabled-启用, paused-暂停 | 否 | [array] | ["enabled","paused"] |

## 请求cURL示例
```
curl --location 'https://openapi.lingxing.com/basicOpen/multiplatform/ads/reportKeywordSvList?access_token=value&timestamp=value&sign=value&app_key=value' \
...
| data>>list>>inStoreOrders | 实体店订单数，实体店归因订单数 | 是 | [string] | 87 |
| data>>list>>inStoreOtherSales | 实体店关联销售额，实体店间接归因销售额 | 是 | [string] | 1234.56 |
| data>>list>>inStoreUnitsSold | 实体店销量，实体店归因销量 | 是 | [string] | 234 |
| data>>list>>isApplyTime | 是否应用分时策略 | 是 | [string] | 1 |
| data>>list>>key | 唯一键，用于前端列表展示，取值为keywordId | 是 | [string] | 987654321 |
| data>>list>>keywordBid | 关键词竞价 | 是 | [string] | 1.25 |
| data>>list>>keywordId | 关键词ID | 是 | [string] | 987654321 |
| data>>list>>keywordName | 关键词文本 | 是 | [string] | 手机壳 |
...
| data>>list>>totalCompleteViewOrders | 完整观看订单总数，【仅SV视频广告】 | 是 | [string] | 56 |
| data>>list>>totalCompleteViewUnits | 完整观看销量总数，【仅SV视频广告】 | 是 | [string] | 78 |
| data>>list>>video5SecondViews | 视频5秒观看次数，【仅SV视频广告】 | 是 | [string] | 8901 |
| data>>list>>videoCompleteViews | 视频完整观看次数，【仅SV视频广告】 | 是 | [string] | 1234 |
| data>>list>>videoFirstQuartileViews | 视频第一四分位观看次数，【仅SV视频广告】 | 是 | [string] | 5678 |
| data>>list>>videoImpressions | 视频曝光量，【仅SV视频广告】 | 是 | [string] | 234567 |
| data>>list>>videoMidpointViews | 视频中点观看次数，【仅SV视频广告】 | 是 | [string] | 4567 |
| data>>list>>videoThirdQuartileViews | 视频第三四分位观看次数，【仅SV视频广告】 | 是 | [string] | 3456 |
| data>>list>>videoUnmutes | 视频取消静音次数，【仅SV视频广告】 | 是 | [string] | 2345 |
| data>>list>>viewThroughOrders | 浏览订单数，【仅SV视频广告】 | 是 | [string] | 123 |
| data>>list>>viewThroughSales | 浏览销售额，【仅SV视频广告】 | 是 | [string] | 3456.78 |
| data>>list>>viewThroughUnitsSold | 浏览销量，【仅SV视频广告】 | 是 | [string] | 89 |
| data>>list>>viewableImpressions | 可见曝光量，【仅SV视频广告】 | 是 | [string] | 123456 |
