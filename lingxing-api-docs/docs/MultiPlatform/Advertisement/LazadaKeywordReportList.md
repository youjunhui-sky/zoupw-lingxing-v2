...
| :------------ |:-----| :------------ | :------------ |
| `/basicOpen/lazadaAd/keyword/report/list` | HTTPS | POST | 1 |
...
| filter>>keyword | 关键词（模糊搜索 / 单词搜索时使用） | 否 | [string] |  |
| filter>>keywords | 关键词列表（精确比较时使用，支持批量） | 否 | [array] |  |
| filter>>searchType | 搜索类型：0-模糊查询（默认），1-精确比较，2-单词搜索 | 否 | [double] |  |
| filter>>keywordIds | 关键词ID列表 | 否 | [array] |  |

## 请求cURL示例

```
curl --location 'https://openapi.lingxing.com/basicOpen/lazadaAd/keyword/report/list?access_token=value&timestamp=value&sign=value&app_key=value' \
...
| data>>list>>keywordId | 关键词ID | 是 | [long] |  |
| data>>list>>keyword | 关键词 | 是 | [string] |  |
...
| data>>summary>>keywordId | 关键词ID | 是 | [long] |  |
| data>>summary>>keyword | 关键词 | 是 | [string] |  |
