...
| :------------ |:-----| :------------ | :------------ |
| `/basicOpen/lazadaAd/store/report/list` | HTTPS | POST | 1 |
...
| filter>>isSyncStatusList | 同步状态列表0=停用, 1=启用 | 否 | [array] |  |

## 请求cURL示例

```
curl --location 'https://openapi.lingxing.com/basicOpen/lazadaAd/store/report/list?access_token=value&timestamp=value&sign=value&app_key=value' \
...
| data>>list>>isSyncStatus | 同步状态 | 是 | [double] |  |
...
| data>>summary>>isSyncStatus | 同步状态 | 是 | [double] |  |
