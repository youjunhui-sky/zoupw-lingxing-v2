...
| :------------ |:-----| :------------ | :------------ |
| `/basicOpen/multiplatform/ads/reportPageTypeSbList` | HTTPS | POST | 1 |
...
| pageType | 页面类型列表，String数组，枚举值：browse-浏览, item-商品, search-搜索, topic-主题, category-分类, homepage-首页, other-其他 | 否 | [array] | ["item","search"] |
...
## 请求cURL示例
```
curl --location 'https://openapi.lingxing.com/basicOpen/multiplatform/ads/reportPageTypeSbList?access_token=value&timestamp=value&sign=value&app_key=value' \
...
| data>>list>>acos | 广告成本占比(ACoS)，计算公式：广告花费 / 广告销售额 * 100% | 是 | [string] | 16.3 |
| data>>list>>adGroupId | 广告组ID | 是 | [string] | 456789123 |
| data>>list>>adGroupName | 广告组名称 | 是 | [string] | 热销商品广告组 |
| data>>list>>adSpend | 广告花费，广告投放的总花费金额 | 是 | [string] | 1226.34 |
| data>>list>>advertisedSkuSales | 直接销售额，广告直接点击归因销售额 | 是 | [string] | 6789.12 |
| data>>list>>advertisedSkuUnits | 直接销量，广告直接点击归因销量 | 是 | [string] | 112 |
| data>>list>>advertiserId | 广告账号ID | 是 | [string] | 987654321098765 |
| data>>list>>aov | 平均订单值(AOV)，计算公式：广告销售额 / 广告订单 | 是 | [string] | 68.9 |
| data>>list>>attributedOrders | 广告订单数，归因总订单数 | 是 | [string] | 178 |
| data>>list>>attributedSales | 广告销售额，归因总销售额 | 是 | [string] | 12256.01 |
| data>>list>>attributedUnits | 广告销量，归因总销量 | 是 | [string] | 178 |
...
| data>>list>>cpa | 平均订单成本(CPA)，计算公式：广告花费 / 广告订单 | 是 | [string] | 6.89 |
| data>>list>>cpc | 点击成本(CPC)，计算公式：广告花费 / 点击量 | 是 | [string] | 0.57 |
| data>>list>>ctr | 点击率(CTR)，计算公式：点击量 / 曝光量 * 100% | 是 | [string] | 2.35 |
| data>>list>>cvr | 转化率(CVR)，计算公式：广告订单 / 点击量 * 100% | 是 | [string] | 8.34 |
...
| data>>list>>inStoreAttributedSales | 实体店销售额，实体店归因总销售额 | 是 | [string] | 1234.56 |
| data>>list>>inStoreOrders | 实体店订单数，实体店归因订单数 | 是 | [string] | 34 |
| data>>list>>inStoreOtherSales | 实体店关联销售额，实体店间接归因销售额 | 是 | [string] | 567.89 |
| data>>list>>inStoreUnitsSold | 实体店销量，实体店归因销量 | 是 | [string] | 89 |
| data>>list>>key | 唯一键，用于前端列表展示 | 是 | [string] | 20241112_item_456789123 |
| data>>list>>mpAdvertiserName | 广告账号名称 | 是 | [string] | Walmart广告账号A |
| data>>list>>mpSellerName | 店铺名称 | 是 | [string] | Walmart官方旗舰店 |
| data>>list>>ntbOrderRate | 品牌新买家订单转化率，计算公式：ntbOrders / numAdsClicks * 100% | 是 | [string] | 2.44 |
| data>>list>>ntbOrders | 品牌新买家订单数，首次购买该品牌的买家产生的订单数 | 是 | [string] | 52 |
| data>>list>>ntbOrdersPercent | 品牌新买家订单占比，计算公式：ntbOrders / attributedOrders * 100% | 是 | [string] | 29.2 |
| data>>list>>ntbRevenue | 品牌新买家销售额，首次购买该品牌的买家产生的销售额 | 是 | [string] | 3367.45 |
| data>>list>>ntbRevenuePercent | 品牌新买家销售额占比，计算公式：ntbRevenue / attributedSales * 100% | 是 | [string] | 27.5 |
| data>>list>>ntbUnits | 品牌新买家销量，首次购买该品牌的买家购买数量 | 是 | [string] | 52 |
| data>>list>>ntbUnitsPercent | 品牌新买家销量占比，计算公式：ntbUnits / attributedUnits * 100% | 是 | [string] | 29.2 |
| data>>list>>numAdsClicks | 广告点击量，广告被点击的总次数 | 是 | [string] | 2134 |
| data>>list>>numAdsShown | 曝光次数，广告被展示的总次数 | 是 | [string] | 90789 |
| data>>list>>otherSkuSales | 关联销售额，间接归因销售额 | 是 | [string] | 4567.89 |
| data>>list>>otherSkuUnits | 关联销量，间接归因销量 | 是 | [string] | 66 |
| data>>list>>pageType | 页面类型，枚举值：browse-浏览, item-商品, search-搜索, topic-主题, category-分类, homepage-首页, other-其他 | 是 | [string] | item |
| data>>list>>roas | 投入产出比(ROAS)，计算公式：广告销售额 / 广告花费 | 是 | [string] | 6.12 |
| data>>total | 总记录数，返回符合条件的数据总数 | 是 | [string] | 1245 |
