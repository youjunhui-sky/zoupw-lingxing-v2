...
| :------------ |:-----| :------------ | :------------ |
| `/basicOpen/multiplatform/ads/reportAdItemSbList` | HTTPS | POST | 1 |
...
| searchText | 搜索文本，模糊搜索广告名称（ad_name） | 否 | [string] | 双十一促销广告 |
| searchType | 搜索类型，目前不用传 | 否 | [string] |  |
| status | 广告状态列表，String数组，枚举值：enabled-启用, disabled-禁用 | 否 | [array] | ["enabled","disabled"] |

## 请求cURL示例
```
curl --location 'https://openapi.lingxing.com/basicOpen/multiplatform/ads/reportAdItemSbList?access_token=value&timestamp=value&sign=value&app_key=value' \
...
| data>>list>>acos | 广告成本占比(ACoS)，计算公式：广告花费 / 广告销售额 * 100% | 是 | [string] | 13.02 |
| data>>list>>adGroupId | 广告组ID | 是 | [string] | 456123789 |
| data>>list>>adGroupName | 广告组名称 | 是 | [string] | SB品牌推广组A |
| data>>list>>adGroupStatus | 广告组状态，枚举值：enabled-启用, disabled-禁用, delete-归档 | 是 | [string] | enabled |
| data>>list>>adItemId | 广告项ID，广告项的唯一标识 | 是 | [string] | 5555666677 |
| data>>list>>adName | 广告名称 | 是 | [string] | 双十一爆款商品A |
| data>>list>>adSpend | 广告花费，广告投放的总花费金额 | 是 | [string] | 2185.34 |
| data>>list>>adStatus | 广告状态，枚举值：enabled-启用, disabled-禁用, deleted-已删除 | 是 | [string] | enabled |
| data>>list>>addDate | 添加日期，广告项添加日期 | 是 | [string] | 2024-10-15 |
| data>>list>>advertisedSkuSales | 直接销售额，广告直接点击归因销售额 | 是 | [string] | 10234.56 |
| data>>list>>advertisedSkuUnits | 直接销量，广告直接点击归因销量 | 是 | [string] | 167 |
| data>>list>>advertiserId | 广告账号ID | 是 | [string] | 987654321098765 |
| data>>list>>aov | 平均订单值(AOV)，计算公式：广告销售额 / 广告订单 | 是 | [string] | 68.45 |
...
| data>>list>>attributedOrders | 广告订单数，归因总订单数 | 是 | [string] | 234 |
| data>>list>>attributedSales | 广告销售额，归因总销售额 | 是 | [string] | 16789.23 |
| data>>list>>attributedUnits | 广告销量，归因总销量 | 是 | [string] | 245 |
| data>>list>>benchmarkVal | 基准值，分时策略的基准值 | 是 | [string] | 1.5 |
| data>>list>>bid | 竞价，当前广告项的竞价 | 是 | [string] | 1.15 |
| data>>list>>campaignAndTargetingType | 广告活动和投放类型组合，格式：campaignType+targetingType | 是 | [string] | sba-手动定向 |
| data>>list>>campaignId | 广告活动ID | 是 | [string] | 789456123 |
| data>>list>>campaignName | 广告活动名称 | 是 | [string] | 双十一品牌推广活动 |
...
| data>>list>>cpa | 平均订单成本(CPA)，计算公式：广告花费 / 广告订单 | 是 | [string] | 8.92 |
| data>>list>>cpc | 点击成本(CPC)，计算公式：广告花费 / 点击量 | 是 | [string] | 0.62 |
| data>>list>>ctr | 点击率(CTR)，计算公式：点击量 / 曝光量 * 100% | 是 | [string] | 2.85 |
| data>>list>>cvr | 转化率(CVR)，计算公式：广告订单 / 点击量 * 100% | 是 | [string] | 12.47 |
| data>>list>>entityCreateAt | 创建时间，实体创建时间 | 是 | [string] | 2024-11-12 15:30:00 |
| data>>list>>isApplyTime | 是否应用分时策略，true-已应用, false-未应用 | 是 | [string] | 1 |
| data>>list>>itemId | 商品ID，广告商品的唯一标识 | 是 | [string] | 1234567890 |
| data>>list>>itemImageUrl | 商品图片URL，商品主图链接 | 是 | [string] | https://i5.walmartimages.com/asr/abc123.jpeg |
| data>>list>>itemPageUrl | 商品详情页URL，商品在平台上的详情页链接 | 是 | [string] | https://www.walmart.com/ip/1234567890 |
| data>>list>>key | 唯一键，用于前端列表展示，取值为adItemId或itemId | 是 | [string] | 9876543210 |
| data>>list>>mpAdvertiserName | 广告账号名称 | 是 | [string] | Walmart广告账号B |
| data>>list>>mpSellerName | 店铺名称 | 是 | [string] | Walmart官方专卖店 |
| data>>list>>ntbOrderRate | 品牌新买家订单转化率，计算公式：ntbOrders / numAdsClicks * 100% | 是 | [string] | 4.05 |
| data>>list>>ntbOrders | 品牌新买家订单数，首次购买该品牌的买家产生的订单数 | 是 | [string] | 76 |
| data>>list>>ntbOrdersPercent | 品牌新买家订单占比，计算公式：ntbOrders / attributedOrders * 100% | 是 | [string] | 32.5 |
| data>>list>>ntbRevenue | 品牌新买家销售额，首次购买该品牌的买家产生的销售额 | 是 | [string] | 5234.67 |
| data>>list>>ntbRevenuePercent | 品牌新买家销售额占比，计算公式：ntbRevenue / attributedSales * 100% | 是 | [string] | 31.18 |
| data>>list>>ntbUnits | 品牌新买家销量，首次购买该品牌的买家购买数量 | 是 | [string] | 76 |
| data>>list>>ntbUnitsPercent | 品牌新买家销量占比，计算公式：ntbUnits / attributedUnits * 100% | 是 | [string] | 31.02 |
| data>>list>>numAdsClicks | 广告点击量，广告被点击的总次数 | 是 | [string] | 1876 |
| data>>list>>numAdsShown | 曝光次数，广告被展示的总次数 | 是 | [string] | 65832 |
| data>>list>>otherSkuSales | 关联销售额，间接归因销售额 | 是 | [string] | 6554.67 |
| data>>list>>otherSkuUnits | 关联销量，间接归因销量 | 是 | [string] | 78 |
| data>>list>>reviewReason | 审核拒绝原因，如果审核失败，说明失败原因 | 是 | [string] |  |
| data>>list>>reviewStatus | 审核状态，广告审核状态 | 是 | [string] | approved |
| data>>list>>roas | 投入产出比(ROAS)，计算公式：广告销售额 / 广告花费 | 是 | [string] | 7.68 |
| data>>list>>suggestedBid | 建议竞价，系统推荐的竞价值 | 是 | [string] | 1.25 |
| data>>total | 总记录数，返回符合条件的数据总数 | 是 | [string] | 3450 |
