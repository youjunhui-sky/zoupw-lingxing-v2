...
| :------------ |:-----| :------------ | :------------ |
| `/basicOpen/lazadaAd/item/report/list` | HTTPS | POST | 1 |
...
| filter>>isProductStockEnough | 商品库存是否充足0=否 1=是 | 否 | [double] |  |
| filter>>isAutoCreative | 是否自动创意0=否 1=是 | 否 | [double] |  |
| filter>>adgroupName | 广告组名称 | 否 | [string] |  |
| filter>>itemIds | 广告商品ID列表 | 否 | [array] |  |

## 请求cURL示例

```
curl --location 'https://openapi.lingxing.com/basicOpen/lazadaAd/item/report/list?access_token=value&timestamp=value&sign=value&app_key=value' \
...
| data>>list>>adgroupStatus | 广告组状态1=开启, 0=关闭 | 是 | [double] |  |
| data>>list>>adgroupStatusDisplayName | 广告组状态显示名称 | 是 | [string] |  |
...
| data>>summary>>adgroupStatus | 广告组状态1=开启, 0=关闭 | 是 | [double] |  |
| data>>summary>>adgroupStatusDisplayName | 广告组状态显示名称 | 是 | [string] |  |
