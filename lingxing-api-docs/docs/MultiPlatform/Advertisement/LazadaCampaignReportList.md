...
| :------------ |:-----| :------------ | :------------ |
| `/basicOpen/lazadaAd/campaign/report/list` | HTTPS | POST | 1 |
...
| filter>>campaignType | 广告活动类型manual=手动, auto=自动 | 否 | [string] |  |
| filter>>statusList | 状态列表1=开启, 0=关闭 | 否 | [array] |  |
| filter>>isAccountBalanceEnough | 账户余额是否充足0=否 1=是 | 否 | [double] |  |
| filter>>isDailyBudgetEnough | 日预算是否充足0=否 1=是 | 否 | [double] |  |
| filter>>campaignName | 广告活动名称 | 否 | [string] |  |
| filter>>campaignIds | 广告活动ID列表 | 否 | [array] |  |

## 请求cURL示例

```
curl --location 'https://openapi.lingxing.com/basicOpen/lazadaAd/campaign/report/list?access_token=value&timestamp=value&sign=value&app_key=value' \
...
| data>>list>>statusDisplayName | 状态显示名称 | 是 | [string] |  |
| data>>list>>dailyBudget | 日预算 | 是 | [string] |  |
| data>>list>>startDate | 开始日期 | 是 | [string] |  |
| data>>list>>endDate | 结束日期 | 是 | [string] |  |
| data>>list>>storeId | 店铺ID | 是 | [long] |  |
| data>>list>>storeName | 店铺名称 | 是 | [string] |  |
| data>>list>>activeAdCount | 生效中的广告数 | 是 | [double] |  |
| data>>list>>itemIds | 商品ID列表 | 是 | [array] | 示例值 |
...
| data>>summary>>statusDisplayName | 状态显示名称 | 是 | [string] |  |
| data>>summary>>dailyBudget | 日预算 | 是 | [string] |  |
| data>>summary>>startDate | 开始日期 | 是 | [object] |  |
| data>>summary>>endDate | 结束日期 | 是 | [object] |  |
| data>>summary>>storeId | 店铺ID | 是 | [long] |  |
| data>>summary>>storeName | 店铺名称 | 是 | [string] |  |
| data>>summary>>activeAdCount | 生效中的广告数 | 是 | [double] |  |
