...
| :------------ | :------------ | :------------ | :------------ |
| `/pb/openapi/newad/hsaNegativeKeywords` | HTTPS | POST | 10 |
...
|data|响应数据|是|[array]| |
|data>>campaign_id|广告活动ID|是|[number]|1640591515|
|data>>ad_group_id|广告组ID|是|[number]|1640591516|
|data>>profile_id|亚马逊店铺数字id|是|[number]|121923590660074|
|data>>keyword_id|关键词ID|是|[number]|800004564000137000|
|data>>keyword_text|关键词|是|[string]|keyword1|
|data>>state|状态|是|[string]|enabled|
|data>>match_type|匹配类别|是|[string]|negativePhrase|
|data>>creation_date|创建时间|是|[number]| |
|data>>last_updated_date|最后更新时间|是|[number]| |
|data>>serving_status|服务状态|是|[string]|&nbsp;|

## 附加说明
出参字段ad_group_id返回值为null时，说明一般是比较早的SB广告，还不支持创建多广告组，其否定关键词里面没有广告组id等信息
