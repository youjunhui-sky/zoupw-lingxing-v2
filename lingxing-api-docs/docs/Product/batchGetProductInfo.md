...
| :------------ | :------------ | :------------ | :------------ |
| `/erp/sc/routing/data/local_inventory/batchGetProductInfo` | HTTPS | POST | 1 |

## 请求参数

| 参数名 | 说明 | 必填 | 类型 | 示例 |  
| :------------ | :------------ | :------------ | :------------ | :------------ |
| productIds |产品id，上限100个【产品id 、 产品sku 、SKU识别码 三选一必填】|否| [array] |["10001","10002"]|
| skus|产品SKU，上限100个【产品id 、 产品sku 、SKU识别码 三选一必填】|否|[array]|["ceshi001","测试002"]|
| sku_identifiers|SKU识别码，上限100个上限100个【产品id 、 产品sku 、SKU识别码 三选一必填】|否|[array]|["ceshi001","测试002"]|
...
|response_time|响应时间|是|[string]|2021-03-1911:53:49|
|data|响应数据|是|[array]||
|data>>id|本地产品id|是|[int]||
|data>>product_name|产品名称|是|[string]|00.100|
|data>>sku|产品sku|是|[string]||
|data>>pic_url|上传的图片地址|是|[string]|xxx/xxx/e8f15ed5eb.jpg|
...
|data>>permission_user_info|负责人信息|是|[array]||
...
|data>>is_combo|是否组合产品：0否，1是|是|[int]||
|data>>brand_name|品牌名称|是|[string]||
|data>>category_name|分类名称|是|[string]||
...
|data>>currency|中国官方汇率code|是|[string]||
|data>>cg_opt_username|采购：采购员|是|[string]||
|data>>cg_delivery|采购：交期|是|[int]||
|data>>cg_price|采购：采购成本（人民币）|是|[number]||
|data>>purchase_remark|采购备注|是|[string]| |
|data>>cg_product_material|采购：材质|是|[string]||
|data>>cg_product_length|采购：产品规格（CM）|是|[number]||
|data>>cg_product_width|采购：产品规格（CM）|是|[number]||
|data>>cg_product_height|采购：产品规格（CM）|是|[number]||
|data>>cg_package_length|采购：包装规格（CM）|是|[number]||
|data>>cg_package_width|采购：包装规格（CM）|是|[number]||
|data>>cg_package_height|采购：包装规格（CM）|是|[number]||
|data>>cg_box_length|采购：外箱规格（CM）|是|[number]||
|data>>cg_box_width|采购：外箱规格（CM）|是|[number]||
|data>>cg_box_height|采购：外箱规格（CM）|是|[number]||
|data>>cg_product_net_weight|采购：产品净重（G）|是|[number]||
|data>>cg_product_gross_weight|采购：产品毛重（G）|是|[number]||
...
|data>>bg_customs_export_name|报关：申报品名（中文）【中文报关名】|是|[string]||
|data>>bg_customs_import_name|报关：申报品名（英文）【英文报关名】|是|[string]||
|data>>bg_customs_import_price|报关：申报金额（进口国）【申报单价】|是|[number]||
|data>>bg_export_hs_code|报关：HSCode（出口国）【中国HSCode】|是|[string]||
|data>>bg_import_hs_code|报关：HSCode（进口国）【美国HSCode】|是|[string]||
|data>>bg_tax_rate|【已废弃字段】报关：税率【美国税率】|是|[number]|28.0000|
|data>>qc_standard|质检标准|是|[object]| |
|data>>qc_standard>>custom_qc_template|自定义质检标准|是|[object]| |
|data>>qc_standard>>custom_qc_template>>qc_image|质检图片|是|[object]| |
|data>>qc_standard>>custom_qc_template>>qc_image>>file_id|领星文件id|是|[int]| |
|data>>qc_standard>>custom_qc_template>>qc_image>>customer_url|客户的图片URL|是|[string]| |
...
|data>>supplier_quote>>is_primary|是否为首选供应商：0否，1是|是|[int]||
...
|data>>supplier_quote>>quotes>>is_tax|是否含税：0否，1是|是|[int]||
...
|data>>combo_product_list|组合商品包含单品信息|是|[array]||
...
|data>>clearance>>customs_clearance_price|默认清关单价|是|[float]| |
