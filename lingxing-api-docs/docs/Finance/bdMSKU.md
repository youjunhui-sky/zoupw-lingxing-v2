# 查询利润报表-MSKU
唯一键说明：dataDate+sid+msku+asin+deferredSubStatusCode
<br>dataDate对应按月汇总或按天汇总
## 接口信息
...
| summaryEnabled| 是否按msku汇总返回：<br>false 默认值 <br>true |否 | [boolean] |false|
| orderStatus|交易状态<br>Deferred 已推迟<br>Disbursed 已发放【默认】<br>DisbursedAndPreSettled 已发放（含预结算）<br>All 全部（不包含已发放预结算数据）|否|[string]|"Disbursed"|
...
| data>>records>>deferredSubStatusCode        | [string]  | 延时状态子状态码                           |       |
