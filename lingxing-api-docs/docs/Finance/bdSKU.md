...
| :------------ | :------------ | :------------ | :------------ |
| `/bd/profit/report/open/report/sku/list` | HTTPS | POST | 10 |
...
| searchField |搜索值类型，local_sku | 否 |[string] | local_sku|
| searchValue |搜索的值  |否 | [array] | ["#"] |
| currencyCode|币种code |否 |[string] | CNY  |
| summaryEnabled| 是否按sku汇总返回：<br>false 默认值 <br>true|否 | [boolean] | false  |
...
| data>>records>>storeName                    | [object]      | 店铺 ，单店铺查询为字符串类型，多店铺查询为字符串数组类型                                                         |        |
| data>>records>>sids                         | [string]     | 店铺id，多个以英文逗号分隔                                       |        |
| data>>records>>asins                        | [string]     | 列表行对应的所有asin,多个以英文逗号分隔                         |        |
| data>>records>>country                      | [object]      | 国家中文名，单店铺查询为字符串类型，多店铺查询为字符串数组类型                                                    |        |
| data>>records>>countryCode                  | [object]      | 国家，单店铺查询为字符串类型，多店铺查询为字符串数组类型                                                         |        |
        ...
        "records": [
            {
                ...
                "sharedVineFee": 0.00,
                "totalStorageFee": -6.95,
                "fbaStorageFee": -6.95,
                "sharedFbaStorageFee": 0.00,
                "longTermStorageFee": -10.09,
                "sharedLongTermStorageFee": 10.09,
